...
class BClassifier(nn.Module):
    ...
    def forward(self, feats, c, label=None, epoch=None):  # N x K, N x C
        ...
        A = F.softmax(
            A / torch.sqrt(torch.tensor(Q.shape[1], dtype=torch.float32, device=device)), dim=0)
...
class Attention(nn.Module):
    ...
    def forward(self, x1, x2, context, mask=None):
        ...
        q, q_2, k, v = map(lambda t: rearrange(
            t, 'b n (h d) -> (b h) n d', h=h), (q, q_2, k, v))
...
class MILNet(nn.Module):
    def __init__(self, i_classifier, num_classes=None, margin=0, p1=0, p2=0, CAMEYLON=False):
        ...
        self.prototype = nn.Parameter(torch.randn(
            (1, num_classes, 512), requires_grad=True))
        ...
        self.classification_layer_conv = nn.Conv1d(
            num_classes, num_classes, kernel_size=512, bias=False)
...
def forward(self, x):
    ...
    mean, var = torch.mean(temp[0: num_tiles1], dim=0), torch.std(
        temp[0: num_tiles1], dim=0)
    ...
    if self.CAMEYLON is False:
        if cf[cf_index[0]] + self.margin > cf[cf_index[1]] and mean_max_index == var_min_index:
            ...
            for i in range(self.num_classes):
                if i == cf_index[0]:
                    m_feats.append(
                        feats[m_indices[0: num_tiles1, i]].mean(dim=0, keepdim=True))
                else:
                    m_feats.append(
                        feats[m_indices[0: num_tiles2, i]].mean(dim=0, keepdim=True))
        ...
        if cf[0] + self.margin > cf[1]:
            m_feats = []
            m_feats.append(
                feats[m_indices[0: num_tiles1, 0]].mean(dim=0, keepdim=True))
            m_feats.append(
                feats[m_indices[0: num_tiles2, 1]].mean(dim=0, keepdim=True))
            m_feats = torch.cat(m_feats, dim=0)[None,]
        elif cf[1] > cf[0]:
            m_feats = []
            m_feats.append(
                feats[m_indices[0: num_tiles2, 0]].mean(dim=0, keepdim=True))
            m_feats.append(
                feats[m_indices[0: num_tiles1, 1]].mean(dim=0, keepdim=True))
            m_feats = torch.cat(m_feats, dim=0)[None,]
        else:
            m_feats = feats[m_indices[0: num_tiles2]]
            m_feats = m_feats.mean(dim=0, keepdim=True)
    ...
    feats = feats[None,]
    feats = self.attention(self.LN_0_1(m_feats), self.LN_0_2(
        self.prototype), self.LN_0_3(feats)) + m_feats
