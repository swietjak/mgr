...
class MnistBags(data_utils.Dataset):
    ...
    def _create_bags(self):
        ...
        for i in range(self.num_bag):
            bag_length = np.int32(self.r.normal(
                self.mean_bag_length, self.var_bag_length, 1))
            if bag_length < 1:
                bag_length = 1

            if self.train:
                indices = torch.LongTensor(self.r.randint(
                    0, self.num_in_train, bag_length))
            else:
                indices = torch.LongTensor(
                    self.r.randint(0, self.num_in_test, bag_length))
    ...
    def __getitem__(self, index):
        if self.train:
            ...
            label = [max(self.test_labels_list[index]),
                     self.test_labels_list[index]]
        # print(bag.shape, label[0].shape)
        return bag, label
